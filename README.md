...
# Literature Review
Current interest in this space is fueled by the desire to achieve faster computation at both training and test time[1]. Work by Courbariaux et. al. shows that binary CNN's are able to achieve near state of the art results on permutation-invariant MNIST, CIFAR-10 and SVHN. Similar work by Kim et. al [2] shows that bitwise neural networks offer competitive performance on the MNIST dataset while offering dramatic computational savings. Special care must be taken when designing the loss function for such models as it becomes non-differentiable. While we only explored the difference between neural networks, convolutional neural networks and their binary counterparts, Tu, Zhijun, et al [3] show remarkable results by using Binary NN's to achieve 66.4% top 1 accuracy on the complex ImageNet Dataset. In future work, we hope to replicate these results as well.
